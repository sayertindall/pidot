 ...
 * schedule.ts
 *
 * SubagentScheduler: timer-driven dispatcher of scheduled subagents. Ports
 * schedule.ts (365 lines) from the tintinweb/pi-subagents reference. See
 * SUB-SPEC-v4.md §7 -- this replaces SUB-SPEC-v3's invented generic FIFO
 * scheduler; the reference already has the right shape, wrapping
 * AgentManager.spawn() directly with bypassQueue: true so a due job can't be
 * deferred behind maxConcurrent long-running background agents.
 *
 * Not ported from the reference's schedule.ts: per-job model-string
 * resolution (its `resolveModel(job.model, ctx.modelRegistry)` call). A
 * scheduled job's `model` field is accepted and stored, but not yet resolved
 * to a real Model at fire time -- the spawn falls back to the agent config's
 * own default model. Revisit alongside a real model-resolver module if
 * per-job model override turns out to matter in practice.
 */
import { Cron } from "croner";
import { nanoid } from "nanoid";
import type { ExtensionAPI, ExtensionContext } from "@earendil-works/pi-coding-agent";
import type { AgentManager } from "./runtime.ts";
import type { ScheduleStore } from "./state.ts";
import type { IsolationMode, ScheduleChangeEvent, ScheduledSubagent, SubagentType, ThinkingLevel } from "./types.ts";
...
export class SubagentScheduler {
	...
	/** Bind to a session's store and arm enabled jobs. */
	...
	/** Stop all timers and drop refs. Safe to call repeatedly. */
	stop(): void {
	...
	/** Fire a job: persist running state, spawn (bypassing the concurrency queue), persist
	 * completion. Fire-and-forget -- the timer tick returns immediately. */
	private executeJob(id: string): void {
		...
		let agentId: string;
		try {
			agentId = manager.spawn(pi, ctx, job.subagentType, job.prompt, {
	...
	static parseRelativeTime(s: string): string | null {
		...
		if (!num) return null;
		const ms = Number.parseInt(num, 10) * { s: 1000, m: 60_000, h: 3_600_000, d: 86_400_000 }[unit];
	...
	static parseInterval(s: string): number | null {
		...
		const unit = m[2] as "s" | "m" | "h" | "d";
		if (!num) return null;
		return Number.parseInt(num, 10) * { s: 1000, m: 60_000, h: 3_600_000, d: 86_400_000 }[unit];
	}
}
