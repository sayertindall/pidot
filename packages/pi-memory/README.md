> [!IMPORTANT]
> **V3 update notice:** this extension now uses the new V3 memory model. If you used V2, update your `pi-memory` settings before running this version. V3 does **not** read the old V2 settings or memory format, and you should start a new clean Pi session after upgrading. See [Migrating from V2](#migrating-from-v2).

> [!NOTE]
> The `master` branch is the active development branch and may include unreleased or unstable changes. For stable versions, install the published npm package with `pi install npm:pi-memory`.

# pi-memory

> **Make Pi sessions feel endless.**

`pi-memory` is a Pi extension that keeps long agent sessions coherent across compactions, handoffs, and days of work.
...
`pi-pi-memory` changes the experience: memory work happens as the session progresses, so when compaction time arrives, Pi can move forward quickly.
...
`pi-pi-memory` continuously captures useful session memory while you work.
...
With `pi-pi-memory`, long sessions feel less like racing against the context window and more like working with an agent that can stay with you.
...
`pi-memory` does the important memory work earlier, while the session is still happening.
...
Use `pi-pi-memory` if you use Pi for:
...
```bash
pi install npm:pi-pi-memory
```

Or install from GitHub/local development:

```bash
pi install git:github.com/elpapi42/pi-pi-memory
# or, from a local checkout:
pi install /absolute/path/to/pi-pi-memory
```
...
Settings live under the `pi-memory` namespace in either:
...
When `debugLog` is enabled, debug events are written as local NDJSON files under Pi's agent directory. Normal sessions write to `pi-memory/debug/<session-id>.ndjson`; contexts without a session id fall back to `pi-memory/debug.ndjson`. Debug rows include `sessionId` and per-consolidation `runId`, so a session file can still be filtered to one observer/reflector/dropper run.
...
Inspired by [Mastra's Observational Memory](https://mastra.ai/blog/pi-memory) research.
