...
export function normalizeSourceEntryIds(
	...
	for (let i = 0; i < allowedSourceEntryIds.length; i++) {
		const id = allowedSourceEntryIds[i];
		if (id !== undefined) allowedOrder.set(id, i);
	}
...
export async function runObserver(args: RunObserverArgs): Promise<Observation[] | undefined> {
	const { model, cwd, priorReflections, priorObservations, chunk, allowedSourceEntryIds, signal } = args;
...
${conversation}`;
	...
	// Create custom resource loader with observer system prompt
	const resourceLoader = new DefaultResourceLoader({
		cwd,
		agentDir: "", // Use default
	...
	// Create in-memory session using createAgentSession
	const { session } = await createAgentSession({
		...
		resourceLoader,
		tools: [recordObservations.name],
		customTools: [recordObservations as any],
		noTools: "all",
	});
