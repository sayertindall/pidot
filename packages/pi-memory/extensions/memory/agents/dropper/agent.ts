...
export function selectDropCandidates(
	...
	for (let i = 0; i < ids.length; i++) {
		const id = ids[i];
		if (id !== undefined && !firstProposalIndex.has(id)) firstProposalIndex.set(id, i);
	}
...
export async function runDropper(args: RunDropperArgs): Promise<string[] | undefined> {
	const { model, cwd, reflections, observations, targetTokens, signal } = args;
	...
	// Create custom resource loader with dropper system prompt
	...
	const { session } = await createAgentSession({
		...
		resourceLoader,
		tools: [dropObservations.name],
		customTools: [dropObservations as any],
		noTools: "all",
	});
