...
export function normalizeSupportingObservationIds(
	...
	for (let i = 0; i < allowedObservationIds.length; i++) {
		const id = allowedObservationIds[i];
		if (id !== undefined && !allowedOrder.has(id)) allowedOrder.set(id, i);
	}
...
export async function runReflector(args: RunReflectorArgs): Promise<Reflection[] | undefined> {
	const { model, cwd, reflections, observations, signal } = args;
	...
	// Create custom resource loader with reflector system prompt
	...
	const { session } = await createAgentSession({
		...
		resourceLoader,
		tools: [recordReflections.name],
		customTools: [recordReflections as any],
		noTools: "all",
	});
