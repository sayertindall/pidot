...
describe("debug logging", () => {
	...
	it("writes enabled events to a session-scoped debug file with session metadata", () => {
		...
		const logPath = join(agentDir, "pi-memory", "debug", "session-123.ndjson");
	...
	it("appends multiple runs for the same session to the same debug file", () => {
		...
		const logPath = join(agentDir, "pi-memory", "debug", "session-abc.ndjson");
	...
	it("uses different debug files for different sessions", () => {
		...
		expect(existsSync(join(agentDir, "pi-memory", "debug", "session-a.ndjson"))).toBe(true);
		expect(existsSync(join(agentDir, "pi-memory", "debug", "session-b.ndjson"))).toBe(true);
	});
	...
	it("sanitizes session ids before using them as filenames", () => {
		...
		expect(debugLogRelativePath({ sessionId: " session/../id:value " })).toBe(join("pi-memory", "debug", "session_.._id_value.ndjson"));
	...
	it("does not write logs when disabled", () => {
		...
		expect(existsSync(join(agentDir, "pi-memory"))).toBe(false);
	});
});
