...
async function execute(id: string, entries: TestEntry[]) {
	...
	const result = await recallObservationTool.execute("tool-1", { id }, undefined as any, undefined as any, ctx as any) as any;
	const text = result.content.filter((part: { type: string; text: string }): part is { type: "text"; text: string } => part.type === "text").map((part: any) => part.text).join("\n");
