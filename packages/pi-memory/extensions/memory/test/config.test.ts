...
import { DEFAULTS, loadConfig, readEnvConfig, resolveCompactAfterTokens } from '../config.js';
...
describe("V3 config", () => {
	...
	it("merges global, project, and env V3 settings in order", () => {
		...
		expect(loadConfig(cwd, { PI_MEMORY_PASSIVE: "true" })).toMatchObject({
			observeAfterTokens: 100,
			reflectAfterTokens: 20,
			compactAfterTokens: 30,
			observationsPoolMaxTokens: 40,
	...
	it("parses passive env override", () => {
		expect(readEnvConfig({ PI_MEMORY_PASSIVE: "on" })).toEqual({ passive: true });
		expect(readEnvConfig({ PI_MEMORY_PASSIVE: "0" })).toEqual({ passive: false });
		expect(readEnvConfig({ PI_MEMORY_PASSIVE: "maybe" })).toEqual({});
	});
