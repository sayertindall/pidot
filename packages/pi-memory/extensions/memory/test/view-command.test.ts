...
function setup(entries: TestEntry[], clipboardResult = true) {
	...
	const run = async (args: unknown = []) => {
		...
		const lastCopy = copyToClipboard.mock.calls.at(-1) as [string] | undefined;
		return {
			output: (lastNotify ? lastNotify[0] : "") as string,
			clipboardText: lastCopy?.[0] as string | undefined,
			copyToClipboard,
		};
	};
