...
describe("V3 dropper agent", () => {
	...
	it("uses active observation count for target-return max drops", () => {
		const observations = [
	...
	it("passes target-return max drops as a hard upper bound", async () => {
		let userText = "";
		promptHandler = async (_tools, text) => {
			userText = text;
		};
		await runDropper({ ...baseArgs });
	...
	it("normalizes active drop ids, filters invalid ids, dedupes", () => {
	...
	it("selects final candidates by coverage, lower relevance, age, then stable ordering", () => {
		...
		expect(selectDropCandidates([
			"aaaaaaaaaaaa", "missing", "111111111111", "bbbbbbbbbbbb",
			"dddddddddddd", "bbbbbbbbbbbb", "eeeeeeeeeeee", "ffffffffffff",
	...
	it("returns capped coverage-preferred proposed observation ids", async () => {
		promptHandler = async (tools) => {
			await tools[0].execute("tool-1", { ids: ["aaaaaaaaaaaa", "missing", "bbbbbbbbbbbb"] });
		};
		await expect(runDropper({ ...baseArgs })).resolves.toEqual(["aaaaaaaaaaaa"]);
	});

	it("returns critical proposed ids when they are the selected valid candidates", async () => {
		promptHandler = async (tools) => {
			await tools[0].execute("tool-1", { ids: ["missing", "cccccccccccc"] });
		};
		await expect(runDropper({ ...baseArgs })).resolves.toEqual(["cccccccccccc"]);
	});

	it("returns undefined when only invalid ids are proposed", async () => {
		promptHandler = async (tools) => {
			await tools[0].execute("tool-1", { ids: ["missing"] });
		};
		await expect(runDropper({ ...baseArgs })).resolves.toBeUndefined();
	});

	it("dedupes repeated tool calls and enforces one run-level cap", async () => {
		promptHandler = async (tools) => {
			await tools[0].execute("tool-1", { ids: ["aaaaaaaaaaaa"] });
			await tools[0].execute("tool-2", { ids: ["bbbbbbbbbbbb", "aaaaaaaaaaaa"] });
		};
		await expect(runDropper({ ...baseArgs })).resolves.toEqual(["aaaaaaaaaaaa"]);
	});
	...
	it("skips the model at or below the target", async () => {
		let called = false;
		promptHandler = async () => { called = true; };
