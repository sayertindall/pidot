...
vi.mock("../agents/observer/agent.js", () => ({ runObserver: mockAgents.runObserver }));
vi.mock("../agents/reflector/agent.js", () => ({ runReflector: mockAgents.runReflector }));
vi.mock("../agents/dropper/agent.js", () => ({ runDropper: mockAgents.runDropper }));
...
describe("V3 consolidation trigger", () => {
	...
	it("model resolution failure skips appending and notifies once", async () => {
		...
		runtime.resolveModel.mockResolvedValueOnce({ ok: false, reason: "no model" } as any);
