...
export const DEBUG_LOG_RELATIVE_PATH = join("pi-memory", "debug.ndjson");
export const DEBUG_LOG_SESSION_DIR_RELATIVE_PATH = join("pi-memory", "debug");
...
export function debugLog(event: string, data: Record<string, unknown> = {}): void {
	...
	try {
		...
		// Append-only NDJSON write — not a read-modify-write, so withFileMutationQueue is not required.
		// Multiple async writes could theoretically interleave, but each is a single atomic line append,
		// and the file format (NDJSON with one JSON object per line) tolerates line-level interleaving.
...
function rotateIfNeeded(path: string): void {
	...
	// Use timestamp-based backup per GUIDE §3.8 corrupt-state pattern.
