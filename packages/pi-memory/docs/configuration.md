...
This page documents the current V3 configuration for `pi-memory`.

V3 keeps the existing `pi-memory` settings namespace, but the setting names changed. Old V2 keys are not aliases; they are ignored. If you are upgrading, read [Migrating from V2](#migrating-from-v2).
...
```bash
grep '"event":"dropper' ~/.pi/agent/pi-memory/debug/<session-id>.ndjson | tail -n 50
```
