...
function hashProviderColor(provider: string): RgbColor {
	let hash = 0;
	for (let i = 0; i < provider.length; i++) {
		hash = provider.charCodeAt(i) + ((hash << 5) - hash);
	}
	// Map to a hue in [140, 320] to avoid boring grays and browns
	const hue = 140 + (Math.abs(hash) % 180);
	const s = 0.65;
	const l = 0.65;
...
export default function minimalFooterExtension(pi: ExtensionAPI): void {
	...
	function getCurrentDirectory(contextCwd: string): string {
		...
		const candidates = [cwd, contextCwd, process.env.PWD, process.cwd()].filter(
	...
	function getProviderColor(provider: string | undefined): RgbColor {
		if (!provider) return { r: 142, g: 142, b: 147 };
		const normalized = provider.toLowerCase();
		return PROVIDER_COLORS[normalized] ?? hashProviderColor(normalized);
	}
		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					const statuses = getExtensionStatusValues(
