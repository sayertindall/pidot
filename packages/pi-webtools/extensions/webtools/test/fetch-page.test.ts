...
test("FetchPage returns text responses unchanged", async () => {
	...
	expect(result.value._tag).toBe("Text");
	expect(result.value._tag === "Text" ? result.value.text : "").toBe("Plain text.");
	expect(publicWeb.requests[0]?.accept ?? "").toMatch(/text\/plain/);
});
...
test("FetchPage converts HTML to markdown when requested", async () => {
	...
	expect(result._tag).toBe("ok");
	expect(result.value._tag).toBe("Text");
	expect(result.value._tag === "Text" ? result.value.text : "").toMatch(/# Hello/);
	expect(result.value._tag === "Text" ? result.value.text : "").toMatch(/World/);
});
...
test("FetchPage returns raster image content", async () => {
	...
	expect(result._tag).toBe("ok");
	expect(result.value._tag).toBe("Image");
	expect(result.value._tag === "Image" ? result.value.data.toString("base64") : "").toBe("AQID");
});
