...
description: Architect Effect/TypeScript programs as call graphs before writing code — name the shapes (§1), draw the happy path A (§2), mark cardinality (§3), annotate error break points E, retry/escape/die (§4), prove dependencies R (§5), trust at schema boundaries (§6), layer behavior with .pipe() (§7), scope resources (§8), swap R for tests (§9), keep gen body = A and pipe = E (§10). Includes the plain-text indented call-graph output format for architecture explanations. Use when designing new features, reviewing whether implementation matches intent, or showing execution flows.
---
